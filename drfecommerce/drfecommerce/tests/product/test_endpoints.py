...
class TestCategoryEndpoints:
    ...
    def test_category_get(self, category_factory, api_client):
        # Arrange
        category_factory.create_batch(4)  # Here we are creating a new category
        # Act
...
class TestBrandEndpoints:
    ...
    def test_category_get(self, brand_factory, api_client):
        # Arrange
        brand_factory.create_batch(4)  # Here we are creating a new category
        # Act
        response = api_client().get(self.endpoint)
        # Assert
        assert response.status_code == 200
...
class TestProductEndpoints:
    ...
    def test_return_all_products(self, product_factory, api_client):
        # Arrange
        product_factory.create_batch(4)  # Here we are creating a new category
        # Act
        response = api_client().get(self.endpoint)
        # Assert
        assert response.status_code == 200
